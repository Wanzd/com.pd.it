	...
	var impl = {
		...
		x$option$line : function(data) {
			...
			$.each(list, function(idx, it) {
						...
						$.each(data.yCol, function(idx2, it2) {
									series[idx2].data.push(list[idx][it2]);
								})
					})
			var option = {
				title : {
				...
				baseOption : {
					...
					tooltip : {
						...
						formatter : function(obj) {
							var value = obj.value;
							return schema[0].text + '：' + value[data.xCol]
									+ schema[1].unit + '<br>' + schema[1].text
						...
						inRange : {
							color : (function() {
								var colors = ['#bcd3bb', '#e88f70', '#edc1a5',
										'#9dc5c8', '#e1e8c8', '#7b7c68',
										'#e5b5b5', '#f0b489', '#928ea8',
										'#bda29a'];
